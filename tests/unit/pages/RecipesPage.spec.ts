...
describe('RecipesPage.vue', () => {
  ...
  it('mostra mensagem vazia e link para criar quando não há receitas', async () => {
    recipesRef.value = []
    const wrapper = mount(RecipesPage, {
      global: {
        stubs: {
          AppHeader: true,
          'router-link': {
            template: '<a :href="to" class="recipes-page__link"><slot /></a>',
            props: ['to'],
          },
        },
      },
  ...
  it('abre modal e deleta e recarrega', async () => {
    ...
    const apiDeleteMock = jest.fn().mockResolvedValue({ ok: true })
    ;(api as any).delete = apiDeleteMock
    ...
    expect(apiDeleteMock).toHaveBeenCalledWith('/recipes/5')
